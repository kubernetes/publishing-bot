...
func (p *PublisherMunger) publish(newUpstreamHeads map[string]plumbing.Hash) error {
	...
	for _, repoRules := range p.reposRules.Rules {
		...
		for _, branchRule := range repoRules.Branches {
			...
			if err := os.WriteFile(path.Join(path.Dir(dstDir), publishedFileName(repoRules.DestinationRepository, strings.Replace(branchRule.Name, "/", "_", 1))), []byte(upstreamBranchHead.String()), 0o644); err != nil {
				return err
			}
		}
	}
