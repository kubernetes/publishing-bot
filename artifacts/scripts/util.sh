...
sync_repo() {
    ...
    for f_mainline_commit in ${f_mainline_commits} FLUSH_PENDING_MERGE_COMMIT; do
        ...
        if [ -n "${k_pending_merge_commit}" ] && [ "${k_new_pending_merge_commit}" != "${k_pending_merge_commit}" ]; then
            ...
            fix-godeps "${deps}" "${is_library}" ${dst_needs_godeps_update} "${commit_msg_tag}"
    ...
    if [ $(git rev-parse HEAD) != "${dst_old_head}" ] || [ "${new_branch}" = "true" ]; then
        fix-godeps "${deps}" "${is_library}" true ${commit_msg_tag}
    else
    ...
    if [ -n "$(git log --oneline --first-parent --merges | head -n 1)" ]; then
        ...
        /collapsed-kube-commit-mapper --repo-name ${source_repo_name} --repo-org ${source_repo_org} --upstream-branch refs/heads/upstream-branch > ../kube-commits-${repo}-${dst_branch}
    else
...
function filter-branch() {
    local commit_msg_tag="${1}" ; shift
    local subdirectory="${2}" ; shift
    local filter="awk 1 && echo && echo '${commit_msg_tag}: "$(echo '${GIT_COMMIT}')"'"
    git filter-branch -f --msg-filter "${filter}" --subdirectory-filter "${subdirectory}" -- "$@"
}
